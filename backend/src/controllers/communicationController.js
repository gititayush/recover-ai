...
function createCommunicationController(repository, whatsappProvider) {
  return {
    ...
    send: async (request, response, next) => {
      try {
        const caseId = request.params.id;
        const result = await dispatchCommunicationAction({
          repository,
          whatsappProvider,
          recoveryCaseId: caseId,
          requestedLanguage: request.body.language || null,
          recipientPhone: request.body.recipientPhone || null,
          channel: request.body.channel || 'whatsapp',
          isAutonomous: false
        });

        if (result.error) {
          return response.status(result.statusCode || 422).json({
            error: result.error,
            message: result.message,
            ...(result.reasons ? { reasons: result.reasons } : {}),
            ...(result.reasonCode ? { reasonCode: result.reasonCode } : {}),
            ...(result.actionDisposition ? { actionDisposition: result.actionDisposition } : {})
          });
        }

        return response.status(result.statusCode || 201).json({
          action: result.action,
          communication: result.action.requestMetadata.communication,
          provenance: result.action.requestMetadata.communication.provenance
        });
      } catch (error) {
        if (error instanceof CommunicationGroundingError || error instanceof UnsupportedLanguageError) {
          return response.status(error.statusCode || 422).json({
